...
// CORREÇÃO: Usar o caminho do banco de dados fornecido pelo ambiente ou o padrão local.
...
const callApiWithRetries = async (apiCallFunction) => {
    let lastError = null;
    for (let i = 0; i < 3; i++) { // Tentar até 3 vezes
        try {
            return await apiCallFunction(); // Se for bem-sucedido, retorna o resultado
        } catch (error) {
            lastError = error;
            const errorMessage = (error.response?.data?.error?.message || error.message || "").toLowerCase();
            const statusCode = error.response?.status;

            // Apenas tentar novamente em erros específicos de servidor ou sobrecarga
            if (errorMessage.includes('overloaded') || errorMessage.includes('server error') || (statusCode >= 500 && statusCode <= 599)) {
                ...
                // Não tentar novamente para erros do cliente (ex: chave de API inválida)
                throw error;
            }
        }
    }
    throw lastError; // Se todas as tentativas falharem, lança o último erro
};
                ...
                try {
                    ...
                    if (response.data.candidates?.[0]?.content?.parts?.[0]) {
                        ...
                        return res.json({
                            ...
                            apiSource: 'Gemini',
                            backupUsed: i > 0 || !!openAIKey // Backup foi usado se OpenAI falhou ou se não for a primeira chave Gemini
                        });
                    }
                } catch (error) {
                    lastError = error;
...
// 9. Inicialização
app.listen(PORT, () => {
    // ATUALIZADO para mostrar a porta correta
