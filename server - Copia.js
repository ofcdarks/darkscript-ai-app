...
const initializeDb = async () => {
    db.serialize(() => {
        ...
        `);
        // Adicionar colunas para compatibilidade com bancos de dados antigos
...
app.post('/api/register', (req, res) => {
    ...
    if (!email || !password || !whatsapp) return res.status(400).json({
        message: 'E-mail, WhatsApp e senha são obrigatórios.'
    });
    ...
    bcrypt.genSalt(10, (err, salt) => {
        bcrypt.hash(password, salt, (err, hash) => {
            ...
            db.run(
                'INSERT INTO users (email, password_hash, whatsapp, settings, is_active) VALUES (?, ?, ?, ?, 0)',
                [email, hash, whatsapp, '{}'],
                function(err) {
                    if (err) {
        ...
        bcrypt.compare(password, user.password_hash, (err, isMatch) => {
            ...
            if (!user.is_active) {
                return res.status(403).json({
                    message: 'A sua conta precisa de ser ativada por um administrador.'
                });
            }
...
app.get('/api/verify-session', verifyToken, (req, res) => {
    db.get('SELECT id, email, role, whatsapp FROM users WHERE id = ?', [req.user.id], (err, user) => {
...
app.put('/api/user/whatsapp', verifyToken, (req, res) => {
    const { whatsapp } = req.body;
    const userId = req.user.id;

    if (!whatsapp) {
        return res.status(400).json({ message: 'O número de WhatsApp é obrigatório.' });
    }
    
    const whatsappRegex = /^\+?[1-9]\d{1,14}$/;
    const cleanedWhatsapp = whatsapp.replace(/\D/g, '');
    if (!whatsappRegex.test(cleanedWhatsapp) || cleanedWhatsapp.length < 10) {
         return res.status(400).json({ message: 'Por favor, insira um número de WhatsApp válido.' });
    }

    db.run('UPDATE users SET whatsapp = ? WHERE id = ?', [whatsapp, userId], function(err) {
        if (err) {
...
const callApiWithRetries = async (apiCallFunction) => {
    ...
    for (let i = 0; i < 3; i++) {
        try {
            ...
            if (errorMessage.includes('overloaded') || errorMessage.includes('server error') || (statusCode >= 500 && statusCode <= 599)) {
                console.log(`Falha na chamada da API (tentativa ${i + 1}), tentando novamente em 2 segundos...`);
...
app.post('/api/generate', verifyToken, (req, res) => {
    ...
    db.get('SELECT settings FROM users WHERE id = ?', [req.user.id], async (err, row) => {
        ...
        if (openAIKey) {
            try {
                const openaiCall = () => axios.post('https://api.openai.com/v1/chat/completions', {
                    model: "gpt-3.5-turbo",
                    messages: [{
                        role: "user",
                        content: prompt
                ...
                const response = await callApiWithRetries(openaiCall);
                const content = response.data.choices[0].message.content;
                return res.json({
                    data: schema ? JSON.parse(content) : {
                        text: content
                    },
                    apiSource: 'OpenAI',
                    backupUsed: false
                ...
                lastError = error;
                console.error("Erro final na OpenAI após retentativas, tentando Gemini:", error.message);
            }
        }
                ...
                try {
                    ...
                    if (response.data.candidates?.[0]?.content?.parts?.[0]) {
                        ...
                        return res.json({
                            ...
                            apiSource: 'Gemini',
                            backupUsed: i > 0 || !!openAIKey
                        });
                    }
                } catch (error) {
                    lastError = error;
                    console.error(`Falha final com a chave Gemini #${i + 1} após retentativas, tentando a próxima:`, error.message);
                }
            }
        }
        ...
        if (lastError) {
            ...
            if (apiError.toLowerCase().includes('quota') || apiError.toLowerCase().includes('billing') || apiError.toLowerCase().includes('api key not valid')) {
                errorMessage = "Todas as chaves de API falharam, possivelmente por falta de créditos/tokens ou chave inválida. Verifique as suas configurações.";
            } else {
...
app.get('/api/admin/users', verifyToken, requireAdmin, (req, res) => {
    ...
    const dataQuery = `SELECT id, email, whatsapp, role, is_active, created_at, last_login_at FROM users WHERE is_active = ? ORDER BY created_at DESC LIMIT ? OFFSET ?`;

    db.get(countQuery, [isActive], (err, row) => {
        if (err) return res.status(500).json({
            message: "Erro ao contar utilizadores."
        });
...
app.put('/api/admin/user/:userId', verifyToken, requireAdmin, (req, res) => {
    ...
    db.run('UPDATE users SET email = ?, whatsapp = ? WHERE id = ?', [email, whatsapp, userId], function(err) {
        if (err) {
            ...
            return res.status(500).json({ message: 'Erro interno do servidor ao atualizar o utilizador.' });
        }
        if (this.changes === 0) {
...
app.put('/api/admin/user/:userId/role', verifyToken, requireAdmin, (req, res) => {
    ...
    if (!role || (role !== 'admin' && role !== 'user')) {
        return res.status(400).json({
            message: 'Cargo fornecido é inválido.'
        });
    }

    if (userId === '1' || userId === req.user.id.toString()) {
        return res.status(403).json({
            message: 'O cargo deste utilizador não pode ser alterado.'
        });
    }
    ...
    db.run('UPDATE users SET role = ? WHERE id = ?', [role, userId], function(err) {
        if (err) {
            return res.status(500).json({
                message: "Erro interno do servidor ao atualizar o cargo."
            });
        }
        if (this.changes === 0) {
        ...
        res.json({
            message: 'Cargo do utilizador atualizado com sucesso.'
        });
    });
});
...
app.post('/api/admin/approve-all', verifyToken, requireAdmin, (req, res) => {
    db.run("UPDATE users SET is_active = 1 WHERE is_active = 0", function(err) {
        ...
        res.json({
            message: `${this.changes} utilizador(es) pendente(s) foram aprovados.`
        });
    });
});
...
app.delete('/api/admin/user/:userId', verifyToken, requireAdmin, (req, res) => {
    ...
    if (userId === '1' || userId === req.user.id.toString()) {
        return res.status(403).json({
            message: 'Este utilizador não pode ser excluído.'
        });
    }
...
app.post('/api/admin/maintenance', verifyToken, requireAdmin, (req, res) => {
    ...
    db.run("REPLACE INTO app_status (key, value) VALUES ('maintenance', ?)", [value], (err) => {
        if (err) return res.status(500).json({
            message: "Erro ao atualizar o modo de manutenção."
        });
        res.json({
            message: 'Modo de manutenção atualizado com sucesso.'
        });
    });
});
...
app.post('/api/admin/announcement', verifyToken, requireAdmin, (req, res) => {
    ...
    db.run("REPLACE INTO app_status (key, value) VALUES ('announcement', ?)", [value], (err) => {
        ...
        res.json({
            message: 'Anúncio global atualizado com sucesso.'
        });
    });
});
